...
<img src="https://33uee5uclf.ufs.sh/f/fa600f97-6cf2-4759-8641-3093d13591bf-6rrk6b.png" alt="LEXEL" width="720" />
...
- **21 Built-in Voices**  - Pre-seeded system voices across 12 categories and 5 locales; optional **Multilingual** voice for 23 languages (female default)
...
Seeds 21 built-in voices (20 standard + **Multilingual**) to the database and R2. The system voice WAV files are included in the repository and originate from [Modal's voice sample pack](https://modal-cdn.com/blog/audio/chatterbox-tts-voices.zip). For the **Multilingual** voice (23 languages with one female voice), add a WAV to `scripts/system-voices/Multilingual.wav` (e.g. copy `Abigail.wav` for the default female voice).

**Optional: 23-language TTS (Chatterbox Multilingual)**  
To enable the Multilingual voice (one voice, 23 languages), deploy the second Modal app and set `CHATTERBOX_MULTILINGUAL_API_URL` in `.env`:

```bash
modal deploy chatterbox_multilingual_tts.py
```

Use the resulting Modal URL as `CHATTERBOX_MULTILINGUAL_API_URL`. The original `chatterbox_tts.py` (Turbo, voice cloning) stays unchanged.
...
LEXEL is designed to be self-hosted. You'll need:
