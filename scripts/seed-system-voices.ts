...
const systemVoiceMetadata: Record<string, VoiceMetadata> = {
  ...
  // Multilingual: use a female WAV (e.g. copy Abigail.wav to Multilingual.wav in system-voices/)
  Multilingual: {
    description: "One voice, 23 languages. Female default for global TTS (Chatterbox Multilingual).",
    category: "GENERAL",
    language: "en",
  },
};
...
async function main() {
  ...
  for (const name of CANONICAL_SYSTEM_VOICE_NAMES) {
    const wavPath = path.join(SYSTEM_VOICES_DIR, `${name}.wav`);
    try {
      await fs.access(wavPath);
    } catch {
      if (name === MULTILINGUAL_VOICE_NAME) {
        console.log(`- ${name} (skipped: add scripts/system-voices/Multilingual.wav, e.g. copy Abigail.wav)`);
        continue;
      }
      throw new Error(`Missing system voice file: ${wavPath}`);
    }
    console.log(`- ${name}`);
